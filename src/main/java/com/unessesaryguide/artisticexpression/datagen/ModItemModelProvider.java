...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    protected void registerModels() {
        withExistingParent("giant_candle", "artisticexpression:block/giant_candle");
    ...
    private <T extends Item> void addColoredBlockItemModels(
        ...
        for (DyeColor color : DyeColor.values()) {
            ...
            // Check for the texture, not the model — textures are hand-made,
            // models are generated, so only textures reliably exist at datagen time
            if (!existingFileHelper.exists(
                ResourceLocation.fromNamespaceAndPath(ArtisticExpression.MODID, "textures/block/" + itemName + ".png"),
                net.minecraft.server.packs.PackType.CLIENT_RESOURCES)) {
                LOGGER.warn("[Artistic Expression] Missing texture for '{}', skipping item model", itemName);
                continue;
            }

            // Use UncheckedModelFile since the block model is generated by
            // ModBlockStateProvider in the same datagen run (not yet on disk)
            getBuilder(itemName)
